...
pub struct MinaPublicKey(CurvePoint);
...
impl PublicKey for MinaPublicKey {
    ...
    fn from_secret_key(secret_key: &Self::SecretKey) -> Self {
        Self::from_secret_key(secret_key.clone()).unwrap()
    }

    fn to_address(
        &self,
        format: &Self::Format,
    ) -> std::result::Result<Self::Address, AddressError> {
        Self::Address::from_public_key(self, format)
    }
}

impl FromStr for MinaPublicKey {
...
impl MinaPublicKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(MinaPublicKey::from_point_unsafe(public))
    }
    ...
    pub fn from_hex(public_hex: &str) -> Result<Self> {
        ...
        MinaPublicKey::from_bytes(&bytes)
    }
    ...
    pub fn from_secret_key(secret_key: MinaSecretKey) -> Result<Self> {
        ...
        Ok(MinaPublicKey::from_point_unsafe(pt))
    }
...
impl fmt::Display for MinaPublicKey {
...
impl CompressedPubKey {
    ...
    /// Deserialize Mina address into compressed public key (via an uncompressed `MinaPublicKey`)
    ///
    /// # Errors
    ///
    /// Will give error if `MinaPublicKey::from_address()` returns error.
    pub fn from_address(address: &str) -> Result<Self> {
        Ok(MinaPublicKey::from_address(address)?.into_compressed())
    }
...
impl From<PubKeyError> for PublicKeyError {
    fn from(err: PubKeyError) -> Self {
        Self::Crate("PubkeyError", format!("{:?}", err))
    }
}

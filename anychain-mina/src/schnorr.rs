...
use crate::hasher::{roinput::ROInput, DomainParameter, Hasher};
use ark_ec::{
...
use crate::{
    BaseField, CurvePoint, Hashable, Keypair, MinaPublicKey, ScalarField, Signature, Signer,
};
...
impl<H: 'static + Hashable> Signer<H> for Schnorr<H> {
    ...
    fn verify(&mut self, sig: &Signature, public: &MinaPublicKey, input: &H) -> bool {
...
pub(crate) fn create_legacy<H: 'static + Hashable>(domain_param: H::D) -> impl Signer<H> {
    Schnorr::<H> {
        hasher: Box::new(crate::hasher::create_legacy::<Message<H>>(
            domain_param.clone(),
        )),
        domain_param,
    }
...
pub(crate) fn create_kimchi<H: 'static + Hashable>(domain_param: H::D) -> impl Signer<H> {
    Schnorr::<H> {
        hasher: Box::new(crate::hasher::create_kimchi::<Message<H>>(
            domain_param.clone(),
        )),
        domain_param,
    }
...
impl<H: 'static + Hashable> Schnorr<H> {
    ...
    fn message_hash(&mut self, pub_key: &MinaPublicKey, rx: BaseField, input: &H) -> ScalarField {
